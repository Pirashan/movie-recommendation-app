...
if (!supabaseUrl || !supabaseServiceKey) { console.error("Missing Supabase env vars"); process.exit(1); }
...
async function calculateAndStoreTfidf() {
  ...
  let allItems: { id: number; media_type: string; combined_text: string | null }[] = [];
  let currentPage = 0;
  let fetchMore = true;
  ...
  const indexToItemKeyMap: { [key: number]: { id: number, media_type: string } } = {};
  ...
  for (let i = 0; i < totalDocs; i++) {
      ...
      if (!itemKey) { console.warn(`Could not find item key for index ${i}`); continue; }
