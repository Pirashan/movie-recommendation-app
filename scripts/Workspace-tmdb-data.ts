...
async function fetchGenreMap(): Promise<Map<number, string>> {
    ...
    const [movieResult, tvResult] = await Promise.all([
        fetchJson<{ genres: { id: number; name: string }[] }>(movieGenresUrl),
        fetchJson<{ genres: { id: number; name: string }[] }>(tvGenresUrl)
    ]);
...
async function fetchKeywords(id: number, mediaType: 'movie' | 'tv'): Promise<string[]> {
    const url = `${TMDB_API_BASE_URL}/${mediaType}/${id}/keywords?api_key=${tmdbApiKey}`;
    const result = await fetchJson<{ id?: number; keywords?: { id: number; name: string }[]; results?: { id: number; name: string }[] }>(url);
    const keywords = result?.keywords || result?.results || [];
...
async function fetchAndStoreMedia() {
    ...
    let itemsToUpsertBatch: any[] = []; // Batch array
    ...
    for (let page = 1; page <= PAGES_TO_FETCH; page++) {
        ...
        const pageResult = await fetchJson<{ results: any[] }>(url);
        if (!pageResult?.results) { console.warn(`No results for Movie Page ${page}.`); continue; }

        for (const item of pageResult.results) {
            totalItemsProcessed++;
            if (!item.id || !item.overview || !item.title || item.adult) { continue; }
            ...
            const genreNames = item.genre_ids?.map((id: number) => genreMap.get(id)).filter(Boolean) as string[] || [];
            ...
            if (!combined) { console.warn(`Skipping movie item due to empty combined text: ID ${item.id}`); continue; }
            ...
            if (itemsToUpsertBatch.length >= ITEMS_PER_BATCH) {
                ...
                if (error) console.error("Supabase movie batch upsert error:", error);
                else console.log("Movie batch upserted successfully.");
    ...
    if (itemsToUpsertBatch.length > 0) {
        ...
        const { error } = await supabase.from('media_items').upsert(itemsToUpsertBatch, { onConflict: 'id, media_type' });
        if (error) console.error("Supabase final movie batch upsert error:", error);
        else console.log("Final movie batch upserted successfully.");
    }
    ...
    console.log("\n--- Fetching Popular TV Shows ---");
     for (let page = 1; page <= PAGES_TO_FETCH; page++) {
        ...
        const pageResult = await fetchJson<{ results: any[] }>(url);
        if (!pageResult?.results) { console.warn(`No results for TV Page ${page}.`); continue; }

         for (const item of pageResult.results) {
            totalItemsProcessed++;
             if (!item.id || !item.overview || !item.name) { continue; }
            ...
            const combined = [
             ...
             if (!combined) { console.warn(`Skipping TV item due to empty combined text: ID ${item.id}`); continue; }
            ...
            if (itemsToUpsertBatch.length >= ITEMS_PER_BATCH) {
                ...
                const { error } = await supabase.from('media_items').upsert(itemsToUpsertBatch, { onConflict: 'id, media_type' });
                if (error) console.error("Supabase TV batch upsert error:", error);
                 else console.log("TV batch upserted successfully.");
                itemsToUpsertBatch = []; // Reset batch
            }
        }
         // Fixed typo: Workspaceed -> Processed
    ...
    if (itemsToUpsertBatch.length > 0) {
        ...
        const { error } = await supabase.from('media_items').upsert(itemsToUpsertBatch, { onConflict: 'id, media_type' });
        if (error) console.error("Supabase final TV batch upsert error:", error);
        else console.log("Final TV batch upserted successfully.");
    }
