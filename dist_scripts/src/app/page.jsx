...
export default function Home() {
    ...
    return (<main className="flex min-h-screen flex-col items-center p-6 pt-12 md:p-12"> {/* More top padding */}
      {/* Search Section */}
      ...
      <div className="w-full max-w-6xl mt-4 px-4"> {/* Wider container, added padding */}
        ...
        {selectedItem && !isLoadingRecommendations && !errorRecommendations && recommendations.length > 0 && (<h2 className="text-2xl font-semibold text-center mb-6">
        ...
        {!isLoadingRecommendations && !errorRecommendations && selectedItem && recommendations.length === 0 && (<p className="text-center text-gray-500">No recommendations found for "{selectedItem.title || selectedItem.name}".</p>)}

        {/* Render RecommendationList Component */}
        {!isLoadingRecommendations && !errorRecommendations && recommendations.length > 0 && (<RecommendationList recommendations={recommendations}/>)}
      </div>
    </main>);
}
