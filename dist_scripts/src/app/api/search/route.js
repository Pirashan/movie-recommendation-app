...
import { searchMedia } from '../../lib/tmdb'; // Use relative path to lib
export async function GET(request) {
    // Get the search query from the URL parameters (e.g., /api/search?query=...)
    const searchParams = request.nextUrl.searchParams;
    const query = searchParams.get('query');
    if (!query) {
        return NextResponse.json({ error: 'Query parameter is required' }, { status: 400 } // Bad Request
        );
    }
    try {
        ...
        return NextResponse.json({ error: 'Failed to fetch search results' }, { status: 500 } // Internal Server Error
        );
    }
}
